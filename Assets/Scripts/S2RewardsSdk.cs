  ...
  public class S2RewardsSdk
  {
...
#if UNITY_EDITOR
    static readonly string FOLDER_ROOT = Path.GetFullPath("Packages/co.squaretwo.rewards-sdk-unity/www~");
#else
    ...
    private static void StartLocalServer() {
      ...
      if (_localWebServer == null) {
        ...
        _localWebServer.logRequests = isLoggingEnabled;
    ...
    private static void InitializeWebView()
    {
      ...
      _webViewObject.Init(
        ...
        err: (msg) =>
        {
          Log(string.Format("CallOnError[{0}]", msg));
        ...
        httpErr: (msg) =>
        {
          Log(string.Format("CallOnHttpError[{0}]", msg));
          _onMessageReceived?.Invoke(new RemoteMessage
          {
            messageId = -1,
            messageType = "WebviewError",
        ...
        hooked: (msg) =>
        {
          Log(string.Format("CallOnHooked[{0}]", msg));
          if (msg != "about:blank")
          {
        ...
        ld: (msg) =>
        {
          Log(string.Format("CallOnLoaded[{0}]", msg));
    ...
    private static async Task<RemoteMessage> SendMessageWithResponse(string messageType, string messageBody = "null", int timeoutMs = 0)
    {
      ...
      Log("SENDING MESSAGE: " + remoteMessageString);
